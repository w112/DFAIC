...
int Cache::checkSkew(MemReq& req, int *raceDetected) {
    // add as true
    bool sidMatch;
    int lineId = array->lookup(req.lineAddr,&req,false,sidMatch);
    return lineId;
}
...
uint64_t Cache::accessSkew(MemReq& req) {
    ...
    if (likely(!skipAccess)) {
        ...
        int32_t lineId = array->lookup(req.lineAddr, &req, updateReplacement, sidMatch);
        
        if(sidMatch)
            respCycle += accLat;
        else
            respCycle = respCycle + accLat + 200;
        ...
        if (lineId == -1 && cc->shouldAllocate(req)) {
            ...
            cc->processEviction(req, wbLineAddr, lineId, respCycle,sid); //1. if needed, send invalidates/downgrades to lower level
        ...
        respCycle = cc->processAccess(req, lineId, respCycle,sid);
...
uint64_t Cache::finishInvalidate(const InvReq& req) {
    ...
    // why there is a nullptr??
    int32_t lineId = array->lookup(req.lineAddr, nullptr, false, sidMatch);
...
bool Cache::finishRefresh(Address lineAddr, uint32_t lineId){
   bool sidMatch = false;
   int32_t lineId_new = array->lookup(lineAddr, nullptr, false,sidMatch);
   assert(lineId_new != -1);
