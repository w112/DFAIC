...
int32_t SetAssocArray::lookup(const Address lineAddr, const MemReq* req, bool updateReplacement, bool& sidMatch) {
...
int32_t CEASERArray::lookup(const Address lineAddr, const MemReq* req, bool updateReplacement,bool& sidMatch) {
    sidMatch = true;
...
int32_t ZArray::lookup(const Address lineAddr, const MemReq* req, bool updateReplacement, bool& sidMatch) {
    /* Be defensive: If the line is 0, panic instead of asserting. Now this can
     * only happen on a s   egfault in the main program, but when we move to full
...
/* FAI-associative array implementation */

FAIArray::FAIArray(uint32_t _numLines, uint32_t _assoc, ReplPolicy* _rp, HashFamily* _hf) : rp(_rp), hf(_hf), numLines(_numLines), assoc(_assoc)  {
    array = gm_calloc<Address>(numLines);
    // init the sid_tag to 0 for these address
    sid_tag = gm_calloc<uint32_t>(numLines);
...
int32_t FAIArray::get_level(const uint32_t sid){
    if(sid == 1 || sid == 2 || sid == 3){
        return 1;
    } else if (sid == 4 || sid == 8 || sid == 12){
        return 2;
    } else {
        return 0;
    }
...
int32_t FAIArray::lookup(const Address lineAddr, const MemReq* req, bool updateReplacement, bool& sidMatch) {
    //  finishInvalidate, should it be 0 for this req ?
    sidMatch = true;
    ...
    for (uint32_t id = first; id < first + assoc; id++) {
        if (array[id] ==  lineAddr) {
            // find the right value in cache array
            /* judge if it a first access or not
                if match, return the data directly, else update the mask bits and return false information
                judge the relationship between sid_tag[id] and sid, 
                high level domains and low level domains
            */   
            /*
            add the tag content switch
            */ 

            int32_t req_level = get_level(sid);
            int32_t tag_level = get_level(sid_tag[id]);

            if(req_level > tag_level){
                sidMatch = true;
            } else if(req_level == tag_level) {
                if((sid_tag[id] & sid) == sid){
                    sidMatch = true;
                }else{
                    // this should judge the level of security domains
                    sidMatch = false;
                    sid_tag[id] |= sid;
                }
            } else {
                // req level < tag_level, do the tag switch
                sidMatch = false;
                sid_tag[id] = sid;
            }
...
uint32_t FAIArray::preinsert(const Address lineAddr, const MemReq* req, Address* wbLineAddr) { //TODO: Give out valid bit of wb cand?
    uint32_t set = hf->hash(0, lineAddr) & setMask;
    uint32_t first = set*assoc;
...
void FAIArray::postinsert(const Address lineAddr, const MemReq* req, uint32_t candidate) {
    rp->replaced(candidate);
    array[candidate] = lineAddr;
    // when insert a new data into cache, we should update a new tag
    sid_tag[candidate] = (*req).sid;
    rp->update(candidate, req);
}