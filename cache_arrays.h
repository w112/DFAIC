...
class CacheArray : public GlobAlloc {
    public:
        ...
        /* Add the interface of sidMatch*/
        virtual int32_t lookup(const Address lineAddr, const MemReq* req, bool updateReplacement, bool& sidMatch) = 0;
...
class SetAssocArray : public CacheArray {
    ...
    public:
        ...
        int32_t lookup(const Address lineAddr, const MemReq* req, bool updateReplacement, bool& sidMatch);
...
/* The first access isolation cache array
   In this cache, the high level domains can access the data of low level domains as the normal cache, 
   However, the access from low level domains to high level domains will get a cache miss in the first time, 
   and update the mask bits of this cache line. 
*/
class FAIArray : public CacheArray {
    protected:
        Address* array;
        uint32_t* sid_tag;
        ReplPolicy* rp;
    ...
    public:
        FAIArray(uint32_t _numLines, uint32_t _assoc, ReplPolicy* _rp, HashFamily* _hf);

        int32_t get_level(const uint32_t sid); 
        int32_t lookup(const Address lineAddr, const MemReq* req, bool updateReplacement, bool& sidMatch);
        uint32_t preinsert(const Address lineAddr, const MemReq* req, Address* wbLineAddr);
        void postinsert(const Address lineAddr, const MemReq* req, uint32_t candidate);
};
