...
uint64_t MESIBottomCC::processEviction(Address wbLineAddr, uint32_t lineId, bool lowerLevelWriteback, uint64_t cycle, uint32_t srcId,uint32_t sid) {
    MESIState* state = &array[lineId];
    ...
    switch (*state) {
        ...
        case E:
            {
                MemReq req = {wbLineAddr, PUTS, selfId,sid, state, cycle, &ccLock, *state, srcId, 0 /*no flags*/};
        ...
        case M:
            {
                MemReq req = {wbLineAddr, PUTX, selfId, sid,state, cycle, &ccLock, *state, srcId, 0 /*no flags*/};
                int bank_num = getParentId(wbLineAddr);
                int num_skews = zinfo->llc_skews;
                int start_bank = bank_num - (bank_num % num_skews);
                //int lineId = -1;
...
uint64_t MESIBottomCC::processAccess(Address lineAddr, uint32_t lineId, AccessType type, uint64_t cycle, uint32_t srcId, uint32_t flags, uint32_t sid) {
    uint64_t respCycle = cycle;
    MESIState* state = &array[lineId];
    switch (type) {
        ...
        case GETS:
            if (*state == I) {
                ...
                uint32_t netLat = 0;
                MemReq req = {lineAddr, GETS, selfId, sid, state, cycle, &ccLock, *state, srcId, flags};
        ...
        case GETX:
            if (*state == I || *state == S) {
                ...
                MemReq req = {lineAddr, GETX, selfId, sid, state, cycle, &ccLock, *state, srcId, flags};
                int bank_num = getParentId(lineAddr);
                int num_skews = zinfo->llc_skews;
                int start_bank = bank_num - (bank_num % num_skews);
                //int lineId = -1;
...
uint64_t MESIBottomCC::processNonInclusiveWriteback(Address lineAddr, AccessType type, uint64_t cycle, MESIState* state, uint32_t srcId, uint32_t flags, uint32_t sid) {
    ...
    MemReq req = {lineAddr, type, selfId, sid, state, cycle, &ccLock, *state, srcId, flags | MemReq::NONINCLWB};
...
uint64_t MESITopCC::processEviction(Address wbLineAddr, uint32_t lineId, bool* reqWriteback, uint64_t cycle, uint32_t srcId, uint32_t sid) {
    if (nonInclusiveHack) {
