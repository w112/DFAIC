...
class FilterCache : public Cache {
    ...
    public:
        ...
        inline uint64_t load(Address vAddr, uint64_t curCycle, uint32_t sid) {
            ...
            if (vLineAddr == filterArray[idx].rdAddr) {
              ...
              return replace(vLineAddr, idx, true, curCycle, sid);
            }
        }

        inline uint64_t store(Address vAddr, uint64_t curCycle, uint32_t sid) {
            Address vLineAddr = vAddr >> lineBits;
            uint32_t idx = vLineAddr & setMask;
            uint64_t availCycle = filterArray[idx].availCycle; //read before, careful with ordering to avoid timing races
            if (vLineAddr == filterArray[idx].wrAddr) {
              ...
              return replace(vLineAddr, idx, false, curCycle, sid);
            }
        }


        uint64_t replace(Address vLineAddr, uint32_t idx, bool isLoad, uint64_t curCycle, uint32_t sid) {
            
            Address pLineAddr = procMask | vLineAddr;
            MESIState dummyState = MESIState::I;
            futex_lock(&filterLock);
            MemReq req = {pLineAddr, isLoad? GETS : GETX, 0,sid, &dummyState, curCycle, &filterLock, dummyState, srcId, reqFlags};
