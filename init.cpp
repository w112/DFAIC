...
BaseCache* BuildCacheBank(Config& config, const string& prefix, g_string& name, uint32_t bankSize, bool isTerminal, uint32_t domain, bool isLLC_1) {
    ...
    if (arrayType == "SetAssoc") {
        ...
        array = new FAIArray(numLines, ways, rp, hf);
    } else if (arrayType == "Z") {
    ...
    if (!isTerminal) {
        ...
        // to support the FAI cache
        if (hashType != "None" || replType != "LRU") panic("Invalid FilterCache config %s", name.c_str());
